...
const UpdateHomeCategoryForm = ({ category, handleClose }: Props) => {
        ...
        onSubmit: (values) => {
            if (category?.id) {
                dispatch(updateHomeCategory({
                    id: category.id,
                    data: { image: values.image, categoryId: values.category3 || values.category2 || values.category },
                }))
            }
            handleClose()
        },
    ...
    const childCategory = (cats: any[], parentCategoryId: any) =>
        cats.filter((child: any) => child.parentCategoryId == parentCategoryId)
    ...
    return (
        <Box component="form" onSubmit={formik.handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            {category && (
                <Box sx={{
                    backgroundColor: '#f7f7f7', border: '1px solid #e0e0e0',
                    borderRadius: '4px', px: 2, py: 1.5,
                    display: 'flex', alignItems: 'center', gap: 2,
                }}>
                    {category.image && (
                        <Box sx={{ width: 48, height: 36, borderRadius: '3px', overflow: 'hidden', border: '1px solid #ddd', flexShrink: 0 }}>
