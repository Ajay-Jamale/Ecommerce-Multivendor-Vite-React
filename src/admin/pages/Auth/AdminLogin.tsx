...
const emailSchema = Yup.object({ email: Yup.string().email('Enter a valid email').required('Email is required') });
...
const AdminLoginForm = () => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setTimer(prev => {
                if (prev <= 1) { clearInterval(interval); setIsTimerActive(false); return 30; }
                return prev - 1;
            });
        }, 1000);
    ...
    const handleSendOtp = async () => {
        ...
        if (errors.email) { formik.setFieldTouched('email', true); return; }
    ...
    return (
        <>
            <style>{`
                ...
                .alf-input {
                    width: 100%; padding: 10px 13px; border: none; background: transparent;
                    font-size: 14px; color: #0F1111; outline: none; border-radius: 8px;
                ...
                .alf-resend-btn {
                    ...
                    color: #007185; font-size: 12px; font-weight: 600;
                ...
                .alf-edit-btn {
                    font-size: 12px; color: #007185; background: none; border: none;
            ...
            <div className="alf-wrap">
                ...
                {/* Email field — hidden once OTP is sent */}
                {!auth.otpSent ? (
                    ...
                    <div className="alf-edit-row">
                        <span className="alf-edit-email">📧 {formik.values.email}</span>
                        <button
                            className="alf-edit-btn"
                            onClick={() => { /* parent can reset otpSent via slice if needed */ }}
                        >
                            Change
                        </button>
                    </div>
